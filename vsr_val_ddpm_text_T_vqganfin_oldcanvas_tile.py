...
def main():
    ...
    opt.savedir = opt.outdir + '/' + opt.dataset
    os.makedirs(opt.savedir, exist_ok=True)

    vqgan_config = OmegaConf.load("configs/vae-decoder/tempdiff_vae.yaml")
            ...
            with precision_scope("cuda"):
                with model.ema_scope():
                    ...
                    for n in trange(len(init_segment_list), desc="Sampling"):
                        ...
                        print("###########################",im_lq_bs.shape, opt.vqgantile_size)
                        ...
                        os.makedirs(os.path.join(opt.savedir, seq_item), exist_ok=True)
                        for jj in range(im_lq_bs.shape[0]):
                            img_name = img_name_list.pop(0)
                            # img_name = str(Path(im_path_bs[jj]).name)
                            basename = os.path.splitext(os.path.basename(img_name))[0]
                            outpath = os.path.join(opt.savedir, seq_item, '{}.png'.format(basename))
