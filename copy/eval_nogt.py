...
if __name__ == '__main__':
    ...
    for video in video_list:
        ...
        for tmp_id, frame in enumerate(frame_list):
            ...
            img_sr = skimage.io.imread(img_sr_path, plugin='pil')
            # PIL_fake = Image.open(path_fake).convert('RGB')
            fake_torch_image = img_to_tensor(img_sr)
            ...
            BRISQUE_this_video.append(brisque_each_img)
            img_srs.append(img_sr)

        # BRISQUE_this_video = [brisque_obj.score(sr) for sr in img_srs]
        NIQE_this_video = [calculate_niqe(sr, crop_border=0, convert_to='y') for sr in img_srs]


        video_niqe = sum(NIQE_this_video) / len(NIQE_this_video)
        # video_brisque = sum(BRISQUE_this_video) / len(BRISQUE_this_video)
        video_brisque = np.mean(BRISQUE_this_video)


        log_str += f'\t # NIQE: {video_niqe:.4f}\t'
        log_str += f'\t # BRISQUE: {video_brisque:.4f}\t'
        opts.logger.info(log_str)
        print(log_str)


        NIQE += video_niqe
        BRISQUE_ += video_brisque

    NIQE /= len(video_list)
    BRISQUE_ /= len(video_list)
    log_str += f"Validation on {sr_root}\n"
    log_str += f'\t # NIQE: {NIQE:.4f}\n'
    log_str += f'\t # BRISQUE: {BRISQUE_:.4f}\n'
    opts.logger.info(log_str)
    print(log_str)
