...
class LatentDiffusionVSRTextWT(DDPM):
    ...
    def instantiate_first_stage(self, config):   #AutoencoderKL
        model = instantiate_from_config(config)
        self.first_stage_model = model.eval()
        self.first_stage_model.train = disabled_train
        for param in self.first_stage_model.parameters():
    ...
    def instantiate_cond_stage(self, config):   #FrozenOpenCLIPEmbedder
        if not self.cond_stage_trainable:
            if config == "__is_first_stage__":
                print("Using first stage also as cond stage.")
                self.cond_stage_model = self.first_stage_model
    ...
    def instantiate_structcond_stage(self, config):   #InflatedEncoderUNetModelWT
        model = instantiate_from_config(config)
        self.structcond_stage_model = model
        self.structcond_stage_model.train()

    def instantiate_flownet_stage(self, config):    #RAFT_SR
        model = instantiate_from_config(config)
    ...
    def p_losses(self, x_start, cond, struct_cond, t, t_ori, z_gt, noise=None):   #gt, c, struc_c, t, t_ori, x, *args, **kwargs
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        # print(x_noisy.shape, t.shape, t_ori.shape)
